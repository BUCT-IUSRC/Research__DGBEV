...
class ImageAug3D:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for depth_map in depth_maps:
            # print("depth_map_shape",depth_map.shape)
            depth_map=torchvision.transforms.ToPILImage()(depth_map.squeeze())
            # print("depth_map",depth_map.size)
            resize, resize_dims, crop, flip, rotate = self.sample_augmentation(data)
            post_rot = torch.eye(2)
            post_tran = torch.zeros(2)
            new_depth_map, rotation, translation = self.img_transform(
                depth_map,
                post_rot,
                post_tran,
                resize=resize,
            ...
            # print("new_depth_map",new_depth_map.size)
            
            tran_totensor=torchvision.transforms.ToTensor()
            new_depth_map=tran_totensor(new_depth_map)
            new_depth_maps.append(new_depth_map)
        data["depth"]=new_depth_maps
        # print("@@@@@@@@@@@@@@@@@@transforms3d")
        for img in imgs:
            # print(img.size)
            resize, resize_dims, crop, flip, rotate = self.sample_augmentation(data)
