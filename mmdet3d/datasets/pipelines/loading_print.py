    ...
    def _load_points(self, lidar_path):
        ...
        if self.load_augmented:
            ...
            if self.load_augmented=="fsnet":
                ...
                new_all_pcl_root="/data/csh_test/vis_out/fsnet1/"
                new_all_pcl_path=os.path.join(new_all_pcl_root,lidar_path[index:])+".pkl.npy"
                new_all_pcl_root2 = "/data/csh_test/vis_out/fsnet5/"
                new_all_pcl_path2 = os.path.join(new_all_pcl_root2, lidar_path[index:]) + ".pkl.npy"
                val_pcl_root="/data/csh_test/vis_out/fsnet5/"
                val_all_pcl_path = os.path.join(val_pcl_root, lidar_path[index:]) + ".pkl.npy"
                flag=0
                if os.path.exists(new_all_pcl_path) or os.path.exists(val_all_pcl_path) or os.path.exists(new_all_pcl_path2):
                    flag=1
                    print("#############################")
                    if os.path.exists(new_all_pcl_path):
                        print("found",new_all_pcl_path)
                        points= np.load(new_all_pcl_path)

                    elif os.path.exists(new_all_pcl_path2):
                        print("found", new_all_pcl_path2)
                        points = np.load(new_all_pcl_path2)
                    elif os.path.exists(val_all_pcl_path):
                        print("found", val_all_pcl_path)
                        points = np.load(val_all_pcl_path)
                # else:
                #     print("@@@@@@@@@@@@@@@not found@@@@@@@@\n",lidar_path)
                    # 创建一个维度为[n, 5]的新数组，最后两个维度都初始化为1
                    points=points.T
                    points_5d = np.ones((points.shape[0], 5))
